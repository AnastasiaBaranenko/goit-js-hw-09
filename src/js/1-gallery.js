...
const allImg = document.createDocumentFragment();

images.forEach(img => {
  const imgList = document.createElement('li');
  imgList.classList.add('gallery-item');
  const imgLinkEl = document.createElement('a');
  imgLinkEl.classList.add('gallery-link');
  imgLinkEl.href = img.original;
  const imagesEl = document.createElement('img');
  imagesEl.classList.add('gallery-image');
  imagesEl.src = img.preview;
  imagesEl.alt = img.description;

  imgLinkEl.append(imagesEl);
  imgList.append(imgLinkEl);
